...
export default function PublicDemoPage() {
  ...
  async function loadFromAccount() {
    ...
    try {
      ...
      setSlotsError("Sign in required to load your services and staff.");
    }
  }
  ...
  return (
    <AppShell>
      <SectionHeader
        ...
        description="This page will mirror the public booking flow for customers."
        action={
          <div className="flex flex-wrap gap-2">
            {user ? (
              <Button variant="outline" onClick={loadFromAccount}>
                Load from my account
              </Button>
            ) : null}
            <a
              href={`/b/${user?.business_id ?? ""}/book`}
              className={user?.business_id ? "" : "pointer-events-none opacity-50"}
            >
              <Button variant="outline">Open booking link</Button>
            </a>
          </div>
        }
      />
          ...
          <CardContent className="space-y-3 text-sm text-muted-foreground">
            {slots.length === 0 && !slotsLoading ? (
              <div className="rounded-lg border border-dashed border-border bg-muted/40 p-4 text-sm text-muted-foreground">
                ...
                <p>
                  Try another date, or confirm staff working hours and time-off
                  settings in the Availability section.
                </p>
              </div>
            ) : null}
